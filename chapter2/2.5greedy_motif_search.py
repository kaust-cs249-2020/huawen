...
def greedy_motif_search(k,t,Dnas):
    '''use greedy algorithm to find the motifs
    k: length of the pattern
    t: the number of dna strings in Dnas
    '''
    best_motif=[]
    motif=[]
...
def profile(motifs):
    ...
    '''
    k=len(motifs[0]) #k is the number of columns in the profile matrix
    s=len(motifs)
    ...
    profile_matrix=profile_matrix/s
    return profile_matrix


def consensus_string_finder(matrix):
    consensus_string=[]
    for i in range(matrix.shape[1]):  #matrix.shape[1] is the column number in matrix
        popletter=[]
        for j in range(len(matrix[:,i])):
            if matrix[:,i][j]==np.max(matrix[:,i]):
                ...
                if po==0:
                    ...
                    popletter="T"
                break
        consensus_string+=popletter
    return ''.join(consensus_string)


def profile_most_probable_kmer(Text,k,profile):
    '''return the kmer with the biggest probability in text,
...
'''
def Score(motifs):
    #return the total distance within the motifs
    n=len(motifs)
    distances=0
    for i in range(len(motifs)):
        pattern=motifs[i]
        for j in range(i+1,n):
            distance = HammingDistance(pattern,motifs[j])
            distances += distance
    return distances

if we define the Score function like this, then we don't need to
calculate the consensus string but its operation time is much larger
'''

Dnas=["GGCGTTCAGGCA","AAGAATCAGTCA","CAAGGAGTTCGC","CACGTCAATCAC","CAATAATATTCG"]
greedy_motif_search(3,5,Dnas)
